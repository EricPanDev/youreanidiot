...
class MainCog(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        embed = Embed(
            ...
            description=f"Help Command: /help\n{shard_info}\nUptime: {datetime.timedelta(seconds=int(round(time.time()-startTime)))}\nSupport: https://discord.gg/qxwTY2x5N7",
            color=discord.Color.green(),
            timestamp=datetime.datetime.now()
        )
        embed.set_footer(text="Powered by BeyondAI • v3.0.2")
        view = discord.ui.View()
        join_guild = discord.ui.Button(style=discord.ButtonStyle.blurple, label="Support Server", url="https://discord.gg/qxwTY2x5N7")
    ...
    async def generate_art(self, interaction: discord.Interaction, prompt: str, style: app_commands.Choice[str] = "default"):
        if not database.is_art_limited(interaction.user.id):
            ...
            if not is_flagged:
                ...
                try:
                    image_url = await generate_from_prompt(prompt, style=(style.value if style != "default" else "default"))
    ...
    async def enable(self, ctx):
        ...
        await ctx.reply(f"{self.bot.user.name} Enabled!")
    ...
    async def disable(self, ctx):
        ...
        await ctx.reply(f"{self.bot.user.name} Disabled!")
    ...
    @app_commands.command(name="roleplay", description="Enable/Display Roleplay Mode")
    @app_commands.choices(
        mode=[
    ...
    async def memory(self, interaction: discord.Interaction, choice: app_commands.Choice[str], data:str = None):
        if str(interaction.user.id) in ["746446670228619414", str(self.owner)]:
            ...
            await interaction.response.send_message("This shape has no registered owner. If this is your shape, create a ticket at discord.gg/qxwTY2x5N7 .", ephemeral=True)
        else:
            await interaction.response.send_message("This isn't your shape! You can make your own at discord.gg/qxwTY2x5N7 tho :)", ephemeral=True)
    ...
    @app_commands.choices(
        time=[
            ...
            app_commands.Choice(name='Until I turn it back on', value="""99999999999999999999999999999999999999999"""),
    ])
